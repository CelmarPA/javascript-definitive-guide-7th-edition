...
function withDB(callback) {
    ...
    request.onerror = console.error; // Log an errors
...
// withDB() calls this function if the database has not been initialized 
// yet.
// We set up the database and populate it with data, then pass the 
// database to the callback function.
//
// Our zip code database includes one object store that holds objects 
// like this:
//
// {
// zipcode: "02134",
...
// }
//
// We use the "zipcode" property as the database key and create an index 
// for the city name.
function initdb(db, callback) {
    ...
    // The zipcodes.json data file was generated from CClicensed data from
    ...
    fetch("zipcodes.json") // Make an HTTP GET request
        ...
        .then(zipcodes => { // Get 40k zip code records
            // In order to insert zip code data into the database we need 
            // a transaction object. To create our transaction object, we 
            // need to specify which object stores we'll be using (we 
            // only have one) and we need to tell it that we'll be doing 
            // writes to the database, not just reads:
            ...
            // The best part about the IndexedDB is that object stores
            // are *realy* simple. Here's how we add (or update) our records:
...
// Given a zipe code, use the IndexedBD API to asynchronously look up 
// the city with that zip code, and pass it to the specified callback, or 
// pass null if no city is found.
