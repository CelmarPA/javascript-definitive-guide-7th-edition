// A web application for displaying and exploring the Mandelbrot set

/*
* This class represents a subrectangle of a canvas or image. We use Tiles
* to divide a canvas into regions that can be processed independently by 
* Workers.
*/
class Tile {
    constructor(x, y, width,height) {
        ...
        this.width = width; // of the tile within a larger rectangle.
        this.height = height;
    }
    ...
    // columns and yields numRows*numCols Tile objects to cover the 
    // rectangle.
...
* performing some kind of computation and then posting a single message 
* with the result of that computation.
*
* Given a WorkerPool and message that represents work to be performed, 
* simply call addWork(), with the message as an argument. If there is
* a Worker object that is currently idle, the message will be posted to
* that worker immediately. If there are no idle Worker objects, the
* message will be queued and will be posted to a Worker when one becomes
* available.
*
* addWork() returns a Promise, which will resolve with the message 
* recieved from the work, or will reject if the worker throws an
* unhandled error.
*/
class WorkerPool {
    ...
    // This internal method is called when a worker finishes working, 
    // either by sending a message or by throwing an error.
    ...
    // will resolve with a worker's response when the work is done. The 
    // work is a value to be passed to a worker with postMessage(). If 
    // there is an idle worker, the work message will be sent 
    // immediately. Otherwise it will be queued until a worker is 
    // available.
    addWork(work) {
...
* center of the image. The perPixel property specifies how much the real * and imaginary parts of that complex number changes for each pixel of 
* the image.
* The maxIterations property specifies how hard we work to compute the 
* set.
* Larger numbers require more computation but produce crisper images.
* Note that the size of the canvas is not part of the state. Given cx, 
* cy, and perPixel we simply render whatever portion of the Mandelbrot
* set fits in the canvas at its current size.
*
* Objects of this type are used with history.pushState() and are used to 
* read the desired state from a bookmarked or shared URL.
*/
class PageState {
...
// The program assumes that this <canvas> element is styled so that it is 
// always as big as the browser window.
class MandelbrotCanvas {
    constructor(canvas) {
        // Store the canvas, get its context object, and initialize a WorkerPool
        this.canvas = canvas;
        this.context = canvas.getContext("2d");
        this.workerPoll = new WorkerPool(NUMWORKERS, "mandelbrotWorker.js");

        // Define some properties that we'll use later
        this.tiles = null; // Subregion of the canvas
    ...
    // Set the canvas size and initialize an array of Tile  objects. 
    // This method is called from the constructor and also by the 
    // handleResize()
    ...
    // Mandelbrot set using that new state, and also saves the new state 
    // with history.pushState(). If the first argument is a function that 
    // function will be called with the state object as its argument and 
    // should make changes to the state. If the first argument is an 
    // object, then we simply copy the properties of that object into the 
    // state object. If the optional second argument is false, then the 
    // new state will not be saved. (We do this when calling setState in 
    // response to a popstate event.)
    ...
    render() {
        // Sometimes the user may use the keyboard or mouse to request 
        // renders more quickly than we can perform them. We don't want 
        // to submit all the renders to the worker pool. Instead if we're 
        // rendering, we'll just make a note that a new render is needed, 
        // and when the current render completes, we'll render the 
        // current state, possibly skipping multiple intermediate states.
        ...
        // for the code in mandelbrotWorker.js. Collect the resulting 
        // Promise objects into an array.
        let promises = this.tiles.map(tile => this.workerPoll.addWork({
            tile: tile,
        ...
        // Use Promise.all() to get an array of responses from the array 
        // of promises. Each response is the computation for one of our tiles.
        ...
        this.pendingRender = Promise.all(promises).then(responses => {
            ...
            // Given the max and the min, compute appropriate values in 
            // the color table. Pixels in the set will be colored fully 
            // opaque black. Pixels outside the set will be translucent 
            // black with higher iteration counts resulting in higher 
            // opacity. Pixels with minimum iteration counts will be 
            // transparent and the white background will show through, 
            ...
            if (min === max) { // If all the pixels are the same,
                ...
                // In the normal case where min and max are different, 
                // use a logarithic scale to assign each possible 
                // iteration count an opacity between 0 and 255, and then 
                // use the shift left operator to turn that into a pixel 
                // value.
            ...
            // (First, though, remove any CSS transforms on the canvas 
            // that may have been set by the pointerdown event handler.)
        ...
        .catch((reason) => {
            ...
            // an error here. This shouldn't happen, but this will help 
            // with debugging if it does.
    ...
    // expensive operation that we can't do multiple times a second, so 
    // we use a timer to defer handling the resize until 200ms have 
    // elapsed since the last resize event was received.
    handleResize(event) {
    ...
    // The pointerdown event might be the start of a zoom gesture (a 
    // click or tap) or a pan gesture (a drag). This handler registers 
    // handlers for the pointermove and pointerup events in order to 
    // respond to the rest of the gesture. (These two extra handlers are 
    // removed when the gesture ends with a pointerup.)
    handlePointer(event) {
        // The pixel coordinates and time of the initial pointer down.
        // Because the canvas is as big as the window, these event 
        // coordinates are also canvas coordinates.
        ...
        const pointerMoveHandler = event => {
            ...
            // If the pointer has moved enough or enough time has passed 
            // that this is not a regular click, then use CSS to pan the 
            // display. (We will rerender it for real when we get the 
            // pointerup event.)
        ...
        const pointerUpHandler = event => {
            ...
            const {cx, xy, perPixel} =  this.state;

            // If the pointer moved far enough or if enough time passed, 
            // then this was a pan gesture, and we need to change state to
            // change the center point. Otherwise, the user clicked or 
            // tapped on a point and we need to center and zoom in on that
            // point.
            if (dx > 10 || dy > 10 | dt > 500) {
...
// Finally, here's how we set up the canvas. Note that this JavaScript 
// file is self-sufficient. The HTML file only needs to include this one 
// <script>.
