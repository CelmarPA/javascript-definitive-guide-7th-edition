...
const clientHTML = fs.readFileSync("simple-chat.html");
...
server.on("request", (request, response) => {
    ...
    // Otherwise send a 404 error for any path other that "/chat" or for
...
async function broadcastNewMessage(request, response) {
    // First, read the body od the request to get the user's message
    request.setEnconding("utf-8");
    let body = "";
    ...
    // Give the message data a prefix taht defines it as a "chat" event
