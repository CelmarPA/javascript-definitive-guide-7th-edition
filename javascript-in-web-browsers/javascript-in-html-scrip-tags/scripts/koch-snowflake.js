...
let deg = Math.PI / 180; // For converting degree to radians

// Draw a level-n Koch snowflake fractal on the canvas context c,
// with lower-left corner at (x,y) and side length len.
function snowflake(c, n, x, y, len) {
    c.save(); // Save current tranformation
    c.translate(x,y); // Tranlate origin to starting point
    ...
    c.restore(); // And restore the original transformation

    // Draw a single leg of a level-n koch snowflake.
    // This function leaves the current point at the end of the leg it has
    // draw and translates the coordinate system so the current point is (0, 0).
    ...
    function leg(n) {
        ...
        if (n === 0) { // Nonrecursive case:
            ...
            c.rotate(60 * deg); // Rotate back to the original heading
            leg(n - 1); // Final sub-leg
        }
        c.restore(); // Restore the tranformation
...
snowflake(c, 1, 175, 125, 125); // A level-1 snowflake is a 6-side star
snowflake(c, 2, 325, 125, 125); // etc.
snowflake(c, 3, 475, 125, 125);
snowflake(c, 4, 625, 125, 125); // A level-4 snowflake looks like a snowdlake!
