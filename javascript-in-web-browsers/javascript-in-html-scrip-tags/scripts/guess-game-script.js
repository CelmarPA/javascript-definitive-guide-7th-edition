...
class GameState {
    ...
    render() {
        ...
        // Display feedback base on the user's last guess. The input
        ...
        if (this.guess == null) {
            ...
            input.placeholder = `${this.guess} is to low. Guess again`;
        } else if (this.guess > this.secret) {
            input.placeholder = `${this.guess} is to hight. Guess again`;
        } else {
    ...
    // Return if the state was updated, and false otherwise.
...
window.onpopstate = (event) => {
    gamestate = GameState.fromStateObject(event.state); // Retore the state
