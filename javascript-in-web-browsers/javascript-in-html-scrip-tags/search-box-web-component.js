 ...
 * components is actually quite hard to do well, and there are many 
 * details to get right. The example demonstrates web components and 
 * does some basic keyboard focus handling, but otherwise ignores 
...
/**
 * This class defines a custom HTML <search-box> element that displays 
 * an <input> text input field plus two icons or emoji. By default, it 
 * displays a magnifying glass emoji (indicating search) to the left of 
 * the text field and an X emoji (indicating cancel) to the right of 
 * the text field. It hides the border on the input field and displays 
 * a border around itself, creating the appearance that the two emoji 
 * are inside the  input field. Similarly, when the internal input 
 * field is focused, the focus ring is displayed around the  
 * <search-box>.
 * 
 ...
 * attributes.
 * 
 * <search-box> supports the normal HTML disabled and hidden attributes 
 * and also size and placeholder attributes, which have the same 
 * meaning for this element as they do for the <input> element.
 * 
 * Input events from the internal <input> element bubble up and appear 
 * with their target field set to the <search-box> element.
 * 
 * The element fires a "search" event with the detail property set to 
 * the current input string when the user clicks on the left emoji (the
 * magnifying glass). The "search" event is also dispatched when the 
 * internal text field generates a "change" event (when the text has 
 * changed and the user types Return or Tab).
 * 
 * The element fires a "clear" event when the user clicks on the right 
 * emoji (the X). If no handler calls preventDefault() on the event 
 * then the element * clears the user's input once event dispatch is 
 * complete.
 * 
 * Note that there are no onsearch and onclear properties or 
 * attributes: handlers for the "search" and "clear" events can only be 
