 ...
 * useful to create visualizations of dynamically loaded data.
 * 
 ...
 * 
 * Instead you must use createElementNS(), which takes an XML namespace 
 * string as its first argument. For SVG, that namespace is the literal 
 * string “http://www.w3.org/2000/svg.”
 * 
 * Other than the use of createElementNS(), the pie chart–drawing code in 
 * Example is relatively straightforward. There is a little math to 
 ...
 * pie slices. The element used to display each slice is <path>. This SVG 
 * element describes arbitrary shapes comprised of lines and curves. The 
 * shape description is specified by the d attribute of the <path> 
 ...
 * current point to the coordinates that follow it. This example also 
 * uses the letter A to draw an arc. This letter is followed by seven 
 * numbers describing the arc,
 */

// Drawing a pie chart with JavaScript and SVG
...
function pieChart(options) {
    ...
    // Get lables and values as arrays and add up the values so we know
    // how big the pie is.
    let labels = Object.keys(data);
    let values = Object.values(data);
    let total = values.reduce((x, y) => x + y);

    // Figure out the angles for all the slices. Slice i startsat 
    // angles[i] and ends at angles[i+1]. The angles are measure in radians.
    let angles = [0];
    values.forEach((x, i) => angles.push(angles[i] + x/total * Math.PI));
    
    // Now loop through the slices of the pie
    values.forEach((value, i) => {
        // Compute the two points where our slice intersects the circle
        // These formulas are chosen so that an angle of 0 is at 12 
        // o'clock and positive angles increase clockwise.
        ...
        // This string describes how to draw a sclice of the pie chart:
        ...
        slice.setAttribute("d", path); // Set the path of this slice
        ...
        label.append(`${label[i]} ${value}`); // Add text to label
        chart.append(label); // Add label tothe chart
    });
