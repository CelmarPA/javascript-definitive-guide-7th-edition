...
class Histrogram {
    ...
    // The size of the histogram is just the number od entries in the Mapo.
    get size() { return this.map.size; }

    // To add a key, just incremetn its count in the Map.
    add(key) { this.map.set(key, this.count(key) + 1); }

    // Deleting a key is a little trickier because we have to delete
    // the key from the Map id the count goes back down to zero.
    delete(key) {
