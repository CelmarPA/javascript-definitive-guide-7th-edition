...
class AbstractSet {
    ...
    has(x) { throw new Error("Abastract method"); }
}

/**
 * NotSet is a croncrete subclass of AbstractSet.
 * The members of this set are all values that are not members of some
 * other set. Because it is defines in terms of another set it is not
...
/**
 * RangeSet is a concrete subclass of AbstractSet. Its members are
 ...
 * iterator. the size getter, and the has() methods get these concrete 
 * methods for free.
 */
class AbstractEnumerableSet extends AbstractSet {
    ...
    equals(set) {
        ...
        for(let element of this) {
            // If an element isn't in the orther set,they aren't equal
...
class SingletonSet extends AbstractEnumerableSet {
    ...
    // We implement these three methods, anf inherit isEmpty, equals()
 ...
 * add(), substract(), and intersect() methods on top of those. Note that
...
class BitSet extends AbstractWritableSet {
    ...
    // Tests wheter the specified bit of the specified byte of our
    ...
    remove(x) {
        if (this._valid(x)) { // If the value is valid
            ...
            if (this._has(byte, bit)) { // If that bit is already  set
                this.data[byte] &= BitSet.masks[bit]; // then inset it
    ...
    // (we could be a blot more clever and optimize this substantially)
    *[Symbol.iterator]() {
...
// Some pre-computed values used by the has(), inset() and remove() methods
BitSet.bits = new Uint8Array([1, 2, 4, 8, 16, 32, 64, 128]);
BitSet.masks = new Uint8Array([~1, ~2, ~4, ~8, ~16, ~32, ~64, ~128]);

// Criando um BitSet com tamanho máximo de 10
const bitSet = new BitSet(10);

// Inserindo elementos no BitSet
bitSet.insert(3);
bitSet.insert(6);
bitSet.insert(9);

// Verificando se um elemento está presente no BitSet
console.log(bitSet.has(6)); // Saída: true
console.log(bitSet.has(8)); // Saída: false

// Removendo um elemento do BitSet
bitSet.remove(3);

// Verificando o tamanho atual do BitSet
console.log(bitSet.size); // Saída: 2

// spread operator para obter os elementos 
console.log([...bitSet]) // Saída: [6, 9]