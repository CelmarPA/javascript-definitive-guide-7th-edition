...
// Worker code for computing regions of the Mandelbrot set
...
onmessage = function(message) {
    ...
    // - tile is an object with width and height properties. It specifies 
    // the size of the rectangle of pixels for which we will be computing
    // Mandelbrot set membership.
    // - (x0, y0) is the point in the complex plane that corresponds to 
    // the upper-left pixel in the tile.
    // - perPixel is the pixel size in both the real and imaginary 
    // dimensions.
    ...
    // Next, we create an ImageData object to represent the rectangular 
    // array of pixels, get its internal ArrayBuffer, and create a typed 
    // array view of that buffer so we can treat each pixel as a single 
    // integer instead of four individual bytes. We'll store the number 
    // of iterations for each pixel in this iterations array. (The 
    // iterations will be transformed into actual pixel colors in the 
    // parent thread.)
    ...
    for(let row = 0, y = y0; row < height; row++, y += perPixel) {
        for(let column = 0, x = x0; column < width; column++, x += perPixel) {
            ...
            // Then we repeatedly compute the complex number z(n+1) based 
            // on this recursive formula:
            //      z(0) = c
            //      z(n+1) = z(n)^2 + c
            ...
            iterations[index++] = n; // Remember # iterations for each 
            // pixel.
    ...
    // When the computation is complete, send the results back to the 
    // parent thread. The imageData object will be copied, but the giant 
    // ArrayBuffer it contains will be transferred for a nice performance
    // boost.
